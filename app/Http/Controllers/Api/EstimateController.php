...
class EstimateController extends Controller{
    ...
    public function createEstimate(Request $request){
        ...
        $data = [
           ...
           'top_purlin_kg' => round(Roofling_purlin($estimate->fm_roofing_purlin,$estimate->fc_roofing_purlin,$estimate->span) * $request->cc_span *$request->length_of_building),
           'side_wall_girt_m2' => Side_wall_girt($estimate->fm_total_side_wall_girt,$estimate->fc_total_side_wall_girt,$estimate->span),
           'side_wall_girt_kg' => round(Side_wall_girt($estimate->fm_total_side_wall_girt,$estimate->fc_total_side_wall_girt,$estimate->span)  * $request->cc_span *$request->length_of_building),
           'gable_end_girt_m2' => gable_end_girt($estimate->fm_total_gable_end_girt,$estimate->fc_total_gable_end_girt,$estimate->span),
           'gable_end_girt_kg' => round(gable_end_girt($estimate->fm_total_gable_end_girt,$estimate->fc_total_gable_end_girt,$estimate->span)* $request->cc_span *$request->length_of_building),
           'roofing_sheet_m2' => roofing_sheet($estimate->fm_total_roofing_sheet,$estimate->fc_total_roofing_sheet,$estimate->span),
           'roofing_sheet_kg' => round(roofing_sheet($estimate->fm_total_roofing_sheet,$estimate->fc_total_roofing_sheet,$estimate->span) *$request->cc_span *$request->length_of_building),
           'clading_sheet_m2' => clading_sheet($estimate->fm_total_clading_sheet,$estimate->fc_total_clading_sheet,$estimate->span),
           'clading_sheet_kg' => round(clading_sheet($estimate->fm_total_clading_sheet,$estimate->fc_total_clading_sheet,$estimate->span)*$request->cc_span *$request->length_of_building),
           'sag_rod_m2' => sag_rod($estimate->fm_sag_rod,$estimate->fc_sag_rod,$estimate->span),
           'sag_rod_kg' => round(sag_rod($estimate->fm_sag_rod,$estimate->fc_sag_rod,$estimate->span) * $request->cc_span * $request->length_of_building),
           'stay_brace_m2' => stay_brace($estimate->fm_stay_brace,$estimate->fc_stay_brace,$estimate->span),
           'stay_brace_kg' => round(stay_brace($estimate->fm_stay_brace,$estimate->fc_stay_brace,$estimate->span) * $request->cc_span * $request->length_of_building),
           'anchor_bolt_m2' => anchor_bolt($estimate->fm_anchor_bolt,$estimate->fc_anchor_bolt,$estimate->span),
           'anchor_bolt_kg' => round(anchor_bolt($estimate->fm_anchor_bolt,$estimate->fc_anchor_bolt,$estimate->span) * $request->cc_span * $request->length_of_building),
    ...
    public function  getEstimateHistoryDetails(Request $request){
        $data =  ProjectEstimantion::where('id',$request->id)->get();
