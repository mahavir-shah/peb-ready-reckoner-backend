...
$(document).ready(function(){
	...
	var dataTable1 = $('.sample_data1').DataTable({
		...
		"ajax" : {
            type:"POST",
			url:'{{route("admin.designationArcadeData")}}', 
            headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        }
        }
	...
	$(document).on('click', '.designation-arcade', function() {
		$this=$(this);
		$.ajax({
              type: 'POST',
              url: "{{ route('admin.designationRestore') }}",
              data: {
                  'id': $(this).attr('rel'),
                  "_token": "{{ csrf_token() }}"
              },
              ...
              success: function(data) {
                $this.closest('tr').remove();
				toastr.success('Designation Restored Successfully');
				dataTable.draw();
              }
          });
	});
